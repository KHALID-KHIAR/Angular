...
export class LoginComponent {
  ...
  async onSubmit() {
    ...
    if (this.myForm.valid) {
      ...
      await this.authfb.signUp(this.field['email'].value, this.field['password'].value).then(
        (res: any) => {
          console.log('User created Sucessefuly ', res);
          this.router.navigateByUrl('/charts/line');
        }
      );
    }
  ...
  async Login() {
    ...
    if (this.myForm.valid) {
      // Your Back-End Code here
      await this.authfb.signIn(this.field['email'].value, this.field['password'].value).then(
        (res: any) => {
          console.log("result of SignIn(): ", res);
          console.log("==============================================");
          this.authfb.authState().subscribe(result => {
            console.log('auth State : ', result);
            this.router.navigateByUrl('/demoFirebase/home');
          })
        }
      );
    }
